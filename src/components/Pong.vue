...
export default defineComponent({
  ...
  setup() {
    ...
    let ballVelocity = [Math.cos(direction)*standardVelocity, Math.sin(direction)*standardVelocity]
    let oldBallVelocity = [0, 0]
    let vy = 0
    const playerPosition = ref([0.1, 0.5])
    const startGame = () => {
      if (!gameStarted) setInterval(() => {
        gameStarted = true
        //playerIntersects
        if(ballPosition.value[0] < 0.1 
        && ballPosition.value[0] > 0.07
        && ballPosition.value[1] > playerPosition.value[1] - 0.15 
        && ballPosition.value[1] < playerPosition.value[1] + 0.15) {
          ballVelocity[0] = Math.abs(ballVelocity[0])
          ballVelocity[1] = Math.atan(vy/ballVelocity[0])*standardVelocity
        }

        //wallIntersects
        if(ballPosition.value[0] > 1)
        ...
        if(ballPosition.value[0] < 0){
          ...
          setTimeout(() => {
            const direction = Math.random()*2*Math.PI
            ballVelocity = [Math.cos(direction)*standardVelocity, Math.sin(direction)*standardVelocity]
          }, 1000)
          ballVelocity = [0, 0]
        }
...
<template>
    ...
    @mouseenter="startGame"
    @mousemove="onMouseMove"
    >
      <div class="ball" :style="{ left: ballPosition[0]*100 + '%', top: ballPosition[1]*100 + '%' }"></div>
      <div class="player" :style="{ left: playerPosition[0]*100 + '%', top: playerPosition[1]*100 + '%' }"></div>
    </div>
</template>
